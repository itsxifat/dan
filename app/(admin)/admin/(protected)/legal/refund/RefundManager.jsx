...
function SectionCard({ section, index, total, onChange, onRemove, onMoveUp, onMoveDown }) {
  return (
    <div className="bg-white/[0.025] border border-white/[0.07] rounded-2xl p-5 space-y-4 group">
      {/* Header */}
      ...
      <div>
        ...
        <input
          ...
          placeholder="e.g. Cancellation Timeline"
        />
      </div>
...
export default function RefundManager({ data }) {
  const [isPending, startTransition] = useTransition();
  const [saved, setSaved] = useState(false);
  const [error, setError] = useState("");

  const [form, setForm] = useState({
    title:         data.title         ?? "Refund & Cancellation Policy",
  ...
  function handleSave(e) {
    ...
    startTransition(async () => {
      try {
        await updateLegalDocument("refund", form);
        setSaved(true);
      } catch (err) {
    ...
    <form onSubmit={handleSave} className="space-y-6 max-w-4xl">
      ...
      <div className={CARD}>
        ...
        <div>
          ...
          <textarea className={TEXTAREA} rows={4}
            ...
            placeholder="A brief introduction shown at the top of the Refund Policy page…" />
          <p className="mt-1.5 text-[10px] text-white/25">
            This paragraph is displayed before all sections as an introduction.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h3 className="text-[13px] font-semibold text-white/80">Sections</h3>
