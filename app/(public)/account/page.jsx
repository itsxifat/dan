...
export default async function AccountPage({ searchParams }) {
  ...
  // Pass initial tab from URL so the server picks it up on first render
  const params = await searchParams;
  const validTabs = ["profile", "bookings", "security"];
  const initialTab = validTabs.includes(params?.tab) ? params.tab : "profile";
  ...
  return (
    <div className="min-h-screen bg-[#F7F4F0]">
      ...
      <div className="relative bg-[#0f0c0e] pt-28 pb-14 px-4 overflow-hidden">
        <div className="absolute inset-0 bg-linear-to-br from-[#2a1024]/80 to-[#0f0c0e]" />
        <div className="absolute inset-x-0 top-0 h-px bg-linear-to-r from-transparent via-[#7A2267]/60 to-transparent" />
        ...
        <div className="relative max-w-3xl mx-auto flex items-center gap-5">
          {data.user.image ? (
            <img
              src={data.user.image}
              alt={data.user.name}
              className="w-16 h-16 rounded-full object-cover border-2 border-[#7A2267]/40 shrink-0"
            />
          ) : (
      ...
      <div className="bg-white border-b border-[#EDE5F0]">
        <div className="max-w-3xl mx-auto px-4 py-4 flex items-center justify-center gap-6 sm:gap-10 overflow-x-auto">
