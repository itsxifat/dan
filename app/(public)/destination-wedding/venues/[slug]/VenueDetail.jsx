...
function Lightbox({ images, startIndex, onClose }) {
  ...
  return (
    <motion.div
      ...
      onClick={onClose}
    >
      <div className="absolute top-0 inset-x-0 h-15 sm:h-17 flex items-center justify-between px-6 z-10"
        onClick={(e) => e.stopPropagation()}>
        <span className={`${josefin.className} text-[10px] uppercase tracking-[0.25em] text-white/40`}>
          {idx + 1} / {images.length}
        </span>
        <button onClick={onClose}
          className="w-9 h-9 rounded-full border border-white/15 bg-white/6
            hover:bg-white/12 flex items-center justify-center text-white/55
      ...
      <div className="w-full max-w-5xl mx-auto px-6 pt-17 pb-16" onClick={(e) => e.stopPropagation()}>
        ...
        <>
          <button onClick={(e) => { e.stopPropagation(); prev(); }}
            className="absolute left-4 sm:left-7 top-1/2 -translate-y-1/2 w-11 h-11 rounded-full
              border border-white/15 bg-white/6 hover:bg-white/14
...
export default function VenueDetail({ venue }) {
  ...
  const allImages = [
    ...
    ...(venue.images     ? venue.images.filter((img) => img !== venue.coverImage) : []),
  ];
  ...
  return (
    <div className="min-h-screen bg-[#faf6f0]">
      ...
      {/* ══ TOP BAR ══════════════════════════════════════════════════ */}
      <div className="bg-[#faf6f0] border-b border-[#ede5d8] sticky top-15 sm:top-17 z-40">
        <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 h-12 flex items-center justify-between">
          <Link href="/destination-wedding#venues"
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
          <motion.div variants={stagger} initial="hidden" animate={headerInView ? "show" : "hidden"}>
            ...
            <motion.div variants={fadeUp} className="flex items-center gap-3 mb-5">
              ...
              <svg viewBox="0 0 8 8" width="6" height="6" className="text-[#7A2267]/30">
                <path d="M4 0.5L7.5 4L4 7.5L0.5 4Z" fill="currentColor" />
              </svg>
              <div className="h-px w-10 bg-[#7A2267]/25" />
            </motion.div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
              <div>
                ...
                <motion.h1 variants={fadeUp}
                  ...
                  <em className="not-italic">{venue.name}</em>
                </motion.h1>
              </div>
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 pb-12">
          <motion.div
            ...
            className="relative w-full rounded-3xl overflow-hidden cursor-pointer group
              shadow-[0_4px_40px_rgba(122,34,103,0.1)]"
            style={{ paddingBottom: "50%" }}
            onClick={() => setLightboxIdx(0)}>
            <img src={venue.coverImage} alt={venue.name}
      ...
      <div ref={bodyRef} className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 pb-16">
        <div className="grid lg:grid-cols-[1fr_300px] xl:grid-cols-[1fr_340px] gap-10 lg:gap-14">
          ...
          <div>
            {/* Description — lora serif for romantic feel */}
            {venue.description && (
              <motion.div
                ...
                <p className={`${lora.className} text-[1.15rem] sm:text-[1.22rem] leading-[1.9]
            ...
            {venue.description && venue.features?.length > 0 && (
              <motion.div
                ...
                <svg viewBox="0 0 24 12" width="24" height="12" fill="none" className="text-[#7A2267]/25">
                  <circle cx="4"  cy="6" r="1.5" fill="currentColor" opacity="0.7" />
                  <path d="M12 1.5L15 6L12 10.5L9 6Z" fill="currentColor" />
                  ...
                  {venue.features.map((feat, i) => (
                    <motion.div key={i} variants={fadeUp}
                      ...
                      <div className="mt-1 shrink-0 w-5 h-5 rounded-lg flex items-center justify-center
                        border border-[#7A2267]/15 bg-[#7A2267]/5
                        group-hover:bg-[#7A2267]/12 transition-colors duration-250">
                        <svg viewBox="0 0 8 8" width="7" height="7" className="text-[#7A2267]/50 group-hover:text-[#7A2267]/75 transition-colors">
                          <path d="M4 0.5L7.5 4L4 7.5L0.5 4Z" fill="currentColor" />
                        </svg>
                      </div>
          ...
          <div className="hidden lg:block">
            <motion.div
              ...
              className="sticky top-31">

              <div className="bg-white rounded-2xl border border-[#ede5d8]
                shadow-[0_4px_32px_rgba(122,34,103,0.07)] overflow-hidden">

                {/* Card header with blush tint */}
                <div className="px-5 pt-5 pb-4 border-b border-[#ede5d8]"
                  style={{ background: "linear-gradient(135deg, rgba(122,34,103,0.04) 0%, transparent 60%)" }}>
                  <p className={`${josefin.className} text-[7.5px] uppercase tracking-[0.28em]
                    text-[#1a1309]/30 mb-1.5`}>Venue Details</p>
                  <p className={`${lora.className} text-[1.05rem] text-[#1a1309]/80 italic leading-snug`}>
                    {venue.name}
                  </p>
                </div>
                ...
                {/* CTA */}
                <div className="p-5">
                  <Link href="/destination-wedding#enquiry"
            ...
            {allImages.length === 1 && (
              <motion.div
                ...
                className="relative rounded-2xl overflow-hidden cursor-pointer group
                  shadow-[0_2px_24px_rgba(122,34,103,0.08)]"
              ...
              <div className="grid grid-cols-2 gap-3">
                {allImages.map((img, i) => (
                  <motion.div key={i}
                    ...
                    className="relative rounded-2xl overflow-hidden cursor-pointer group
                      shadow-[0_2px_20px_rgba(122,34,103,0.07)]"
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-12 gap-3">
                  <motion.div
                    ...
                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/7 transition-colors duration-300" />
                  </motion.div>
                  ...
                  <div className="col-span-12 sm:col-span-4 grid grid-rows-2 gap-3">
                    {allImages.slice(1, 3).map((img, i) => (
                      <motion.div key={i}
                        ...
                        className="relative rounded-2xl overflow-hidden cursor-pointer group
                          shadow-[0_2px_16px_rgba(122,34,103,0.07)]"
                    ...
                    {allImages.slice(3).map((img, i) => (
                      <motion.div key={i} variants={fadeUp}
                        className="relative rounded-xl overflow-hidden cursor-pointer group
                          shadow-[0_2px_12px_rgba(122,34,103,0.06)]"
                        style={{ paddingBottom: "100%" }}
                        onClick={() => setLightboxIdx(i + 3)}>
                        <img src={img} alt={`Photo ${i + 4}`}
                          className="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-[1.06]" />
                        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/9 transition-colors duration-250" />
                      </motion.div>
                    ))}
                  </motion.div>
                )}
      ...
      <section ref={ctaRef} className="bg-[#faf6f0] border-t border-[#ede5d8] py-16 sm:py-20">
        <motion.div
          ...
          <div className="flex items-center justify-center gap-4 mb-7">
            ...
            <svg viewBox="0 0 28 14" width="28" height="14" fill="none" className="text-[#7A2267]/30">
              <circle cx="4"  cy="7" r="2"   fill="currentColor" opacity="0.5" />
              <path d="M14 1.5L17.5 7L14 12.5L10.5 7Z" fill="currentColor" />
              <circle cx="24" cy="7" r="2"   fill="currentColor" opacity="0.5" />
            </svg>
            <div className="h-px w-12 bg-[#7A2267]/20" />
          </div>
          ...
          <p className={`${josefin.className} text-[12.5px] text-[#1a1309]/45 font-light leading-[1.9] mb-9 max-w-md mx-auto`}>
            Speak with our dedicated wedding team to check availability and begin crafting your perfect celebration.
          </p>
