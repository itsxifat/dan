  ...
  callbacks: {
    ...
    async jwt({ token, user, account, trigger, session }) {
      if (user) {
        ...
        // Google's user.id is the OAuth sub (numeric string), not a MongoDB ObjectId.
        // If the signIn callback mutation didn't propagate, look up the real _id here.
        if (account?.provider === "google" && !/^[a-f\d]{24}$/i.test(token.id)) {
          await dbConnect();
