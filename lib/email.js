...
import { cdnSignedUrl, isCdnUrl } from "@/lib/cdn";
import { DOC_COPY, docNoticeLines, docNoticeHeadline } from "@/lib/guestDocs";
...
const SITE_URL = (process.env.NEXT_PUBLIC_BASE_URL || process.env.BASE_URL || "").replace(/\/$/, "");

// Mail clients fetch images with no whitelisted Referer, so the CDN logo must be
// a signed (domain-lock-bypassing) URL. Falls back to the on-site logo if the
// CDN logo isn't configured yet.
const CONFIGURED_LOGO = process.env.NEXT_PUBLIC_LOGO_URL || "";
const EMAIL_LOGO_SRC  = isCdnUrl(CONFIGURED_LOGO)
  ? cdnSignedUrl(CONFIGURED_LOGO, { lifetime: true })
  : `${SITE_URL}/logo.png`;
...
// Shared anti-spam headers added to every sendMail call
const BASE_HEADERS = {
  "X-Mailer":       "Dhali's Amber Nivaas Mailer",
  "List-Unsubscribe": `<mailto:${process.env.EMAIL_USER}?subject=unsubscribe>`,
};

// ─── Shared logo block ────────────────────────────────────────────────────────
function logoBlock(siteUrl = SITE_URL) {
  return `
    <a href="${siteUrl}" style="display:block;text-decoration:none;margin:0 auto 18px;text-align:center;">
      <img src="${EMAIL_LOGO_SRC}"
...
// ─── Guest documentation notice ───────────────────────────────────────────────
//
// Rendered into every guest-facing mail that precedes check-in. The wording
// comes from lib/guestDocs so the inbox always matches what the booking wizard,
// the invoice and the Terms & Conditions say. Marriage-certificate lines only
// appear when that booking's rooms actually trigger the rule.
...
function docNoticeHtml(summary, { sectionLabel = "Before You Arrive", heading = DOC_COPY.heading } = {}) {
  const lines = docNoticeLines(summary);
  if (lines.length === 0) return "";

  const roomHint = summary.marriageCertRooms?.length
    ? `<p style="margin:10px 0 0;font-size:11.5px;color:#9A6400;">Applies to room${summary.marriageCertRooms.length > 1 ? "s" : ""} ${escapeHtml(summary.marriageCertRooms.join(", "))}.</p>`
    : "";

  const items = lines
    .map(
      (l) => `<tr>
        <td valign="top" style="padding:0 8px 8px 0;font-size:12.5px;color:#B45309;line-height:1.6;">&bull;</td>
        <td valign="top" style="padding:0 0 8px;font-size:12.5px;color:#92400E;line-height:1.65;">${escapeHtml(l)}</td>
      </tr>`
    )
    .join("");

  return `
    <p style="margin:0 0 12px;font-size:9.5px;letter-spacing:0.18em;text-transform:uppercase;color:#9B8BAB;font-weight:600;border-bottom:1px solid #F0E8F4;padding-bottom:8px;">${escapeHtml(sectionLabel)}</p>
    <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="margin-bottom:24px;">
      <tr>
        <td style="background:#FFFBEB;border:1px solid #FDE68A;border-left:3px solid #D97706;border-radius:10px;padding:16px 18px;">
          <p style="margin:0 0 4px;font-size:13px;font-weight:700;color:#92400E;">${escapeHtml(heading)}</p>
          <p style="margin:0 0 12px;font-size:11.5px;color:#B45309;font-weight:600;">${escapeHtml(docNoticeHeadline(summary))}</p>
...
function docNoticeText(summary) {
  const lines = docNoticeLines(summary);
  if (lines.length === 0) return [];
  return [
    ``,
    `${DOC_COPY.heading.toUpperCase()}`,
    `─────────────────────────────────────`,
    ...lines.map((l) => `- ${l}`),
    summary.marriageCertRooms?.length
      ? `- Applies to room(s): ${summary.marriageCertRooms.join(", ")}`
      : null,
  ].filter(Boolean);
}
...
export async function sendOtpEmail(to, code, purpose) {
  ...
  const headline = purpose === "signup" ? "Email Verification" : "Password Reset";
  ...
  const text = [
    ...
    "",
    `© ${new Date().getFullYear()} Dhali's Amber Nivaas`,
    SITE_URL,
  ].join("\n");
...
<head>
  ...
  <title>${headline} — Dhali's Amber Nivaas</title>
</head>
    ...
    <tr>
      <td align="center">
        <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="max-width:480px;">
          ...
          <!-- Header -->
          <tr>
            <td style="background:#1A0C17;border-radius:16px 16px 0 0;padding:32px 36px 24px;text-align:center;">
              ${logoBlock()}
              ...
              <h1 style="margin:0;font-size:22px;font-weight:600;color:#ffffff;letter-spacing:-0.01em;">${headline}</h1>
            </td>
          </tr>

          <!-- Body -->
          <tr>
            <td style="background:#ffffff;padding:36px 32px 28px;border-radius:0 0 16px 16px;box-shadow:0 8px 40px rgba(0,0,0,0.07);">
              <p style="margin:0 0 24px;color:#555;font-size:14px;line-height:1.7;">${message}</p>

              <!-- OTP Box -->
              <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="margin-bottom:24px;">
                <tr>
                  <td style="background:#F7F4F0;border:2px dashed #D4A8E0;border-radius:12px;padding:24px;text-align:center;">
                    ...
                    <p style="margin:0;color:#7A2267;font-size:40px;font-weight:700;letter-spacing:0.3em;line-height:1;">${code}</p>
                  </td>
                </tr>
              </table>

              <p style="margin:0;color:#9B8BAB;font-size:12px;line-height:1.6;">
                This code expires in <strong style="color:#555;">10 minutes</strong>. Do not share this code with anyone.
              </p>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:20px 32px 0;text-align:center;">
              <p style="margin:0 0 6px;font-size:11px;color:#A898BA;">
                Questions? Contact us at <a href="mailto:${process.env.EMAIL_USER}" style="color:#7A2267;text-decoration:none;">${process.env.EMAIL_USER}</a>
              </p>
...
export async function sendBookingConfirmationEmail({
  ...
  const invoiceBreakdownHtml = subtotal > 0
    ? `<p style="margin:0 0 10px;font-size:9.5px;letter-spacing:0.18em;text-transform:uppercase;color:#9B8BAB;font-weight:600;border-bottom:1px solid #F0E8F4;padding-bottom:8px;">Invoice Breakdown</p>
      <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:20px;">
        <tr><td style="padding:7px 0;border-bottom:1px solid #F5EEF9;">
          <table width="100%" cellpadding="0" cellspacing="0"><tr>
  ...
  // ── Savings badge ──────────────────────────────────────────────────────────
  const savingsHtml = totalSaved > 0
    ? `<tr>
    ...
    <tr>
      <td align="center">
        <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="max-width:520px;">
          ...
          <!-- Header -->
          <tr>
            <td style="background:#1A0C17;border-radius:16px 16px 0 0;padding:32px 36px 26px;text-align:center;">
              ${logoBlock(baseUrl)}
              <p style="margin:0 0 6px;font-size:10px;letter-spacing:0.22em;text-transform:uppercase;color:rgba(255,255,255,0.45);">Dhali's Amber Nivaas</p>
              ...
              <h1 style="margin:0;font-size:26px;font-weight:300;color:#ffffff;letter-spacing:-0.01em;">${propertyName}</h1>
            </td>
          </tr>
          ...
          <tr>
            <td style="background:#ffffff;padding:32px 36px 28px;border-radius:0 0 16px 16px;box-shadow:0 8px 40px rgba(0,0,0,0.07);">
              ...
              <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="margin-bottom:8px;">
                ...
                <tr><td style="padding-bottom:0;">${paymentHtml}</td></tr>
                ${savingsHtml}
              </table>
          ...
          <tr>
            <td style="padding:20px 36px 0;text-align:center;">
              ...
              <p style="margin:0 0 16px;font-size:11px;"><a href="mailto:${process.env.EMAIL_USER}" style="color:#7A2267;text-decoration:none;">${process.env.EMAIL_USER}</a></p>
              <p style="margin:0;font-size:10px;color:#C4B3CE;">© ${new Date().getFullYear()} Dhali's Amber Nivaas. All rights reserved.</p>
            </td>
          </tr>
...
export async function sendCheckInEmail({
  ...
  // At check-in the notice becomes a verification reminder rather than a
  // "bring these with you" list — and only if something is still outstanding.
  const docsOutstanding = Boolean(
    docSummary && (docSummary.marriageCertPending || docSummary.docsDeferred || docSummary.adultsMissingNid > 0)
  );
  const checkInDocSummary = docsOutstanding ? docSummary : null;
  const DOC_SECTION = { sectionLabel: "Document Verification", heading: "Documents to present at the reception desk" };
  ...
  const text = [
    ...
    ...docNoticeText(checkInDocSummary),
    ``,
        ...
        <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="max-width:520px;">
          ...
          <tr>
            <td style="background:#ffffff;padding:32px 36px 28px;border-radius:0 0 16px 16px;box-shadow:0 8px 40px rgba(0,0,0,0.07);">
              ...
              ${docNoticeHtml(checkInDocSummary, DOC_SECTION)}
...
export async function sendCheckOutEmail({
  ...
  const roomsSummary = rooms.length > 0
    ? `<tr>
